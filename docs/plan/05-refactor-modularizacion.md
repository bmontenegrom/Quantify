...
Nota (2026-07-28, cont.): **`db.rs` → hecho** (`db/mod.rs` con DTOs+queries+facade de `courses`/`sessions`/`submissions`/`users`, `db/schema.rs` con `migrate`/`add_column_if_missing`, `db/password.rs` con `hash_password`/`verify_password`/`digest_password`/`VerifyResult`). Tests quedaron inline en `db/mod.rs` (son solo ~17% del archivo, no ameritaba extraerlos aparte).

Nota (2026-07-28, cont. 2): **`submissions.rs` → hecho** (`submissions/mod.rs` con las entregas/informes/miembros + `submissions/gradebook.rs` con los DTOs y funciones de libreta: `GradeComponent`/`GradeScore`/`CourseGradebook`/etc., `create_grade_component`/`upsert_grade_score`/`gradebook_for_user`). Corte limpio, sin dependencias cruzadas más allá de `clean_zero`/`next_position`/`AuthUser`/`Course`. Tests quedan inline en `submissions/mod.rs`.

Nota (2026-07-28, cont. 3): **`courses.rs` → hecho**, split por entidad como proponía el plan: `courses/mod.rs` (`Course`/`CourseSummary`/`AcademicContext` + `academic_context`/`create_course`/`update_course`/`add_course_member`/`enable_course_practice`/`user_can_submit`/`practices_for_course` + el ensamblado `course_summaries`), `courses/group.rs` (`LabGroup`/`GroupSummary`+CRUD de grupos y `enroll_course_member`), `courses/subgroup.rs` (`PracticeSubgroup`/`PracticeTableAssignment`/`SubgroupSummary`+CRUD de subgrupos y mesas). Más cruces que `submissions.rs`: `course_summaries` (mod.rs) llama a `groups_for_course`/`subgroups_for_course`/`table_assignments_for_course` (las dos últimas `pub(super)` en `subgroup.rs`), y `SubgroupSummary` referencia `GroupSummary`/`LabGroup` de `group.rs` vía `super::group::{...}`. Tests quedan inline en `courses/mod.rs`. Falta: `instruments.rs` → `instruments/seed.rs` + `import_export.rs`, y el dedup de `guard_symbol`.
